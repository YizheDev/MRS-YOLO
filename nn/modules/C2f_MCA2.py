...
class C2fWithMCA(nn.Module):
    ...
    def forward(self, x):
        weight1,weight2,weight3 = self.weight/sum(self.weight)
        # 首先对x进行池化特征提取
        x = self.mca(x)
        # 根据分割数量选择不同的卷积核进行卷积操作
        y1 = self.cv11(x)
        y2 = list(self.cv21(x).chunk(2, 1))
        y2.extend(m(y2[-1]) for m in self.m)
        y2 = self.cv22(torch.cat(y2, 1))
        y3 = list(self.cv31(x).chunk(3, 1))
        y3.extend(m(y3[-1]) for m in self.m)
        y3 = self.cv32(torch.cat(y3, 1))
        y = 1/3 * (y1+y2+y3) + weight1 * y1 +weight2 * y2 +weight3 * y3
        return y
