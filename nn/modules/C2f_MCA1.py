...
class PoolAttention(nn.Module):
    def __init__(self, inp, no_spatial=True):
        ...
        self.h_cw = PoolGate(3)
        # 创建宽度维度的自注意力门控模块
        self.w_hc = PoolGate(3)
        self.no_spatial = no_spatial
        # 如果不仅对通道维度进行自注意力操作，则创建通道维度的自注意力门控模块
        if not no_spatial:
            self.c_hw = PoolGate(kernel)
        # 创建可学习的权重参数
...
class C2fChunkPooling(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, threshold1=0.34, threshold2=0.67,splits = 2):
        """初始化CSP块，参数包括输入通道数c1，输出通道数c2，重复次数n，是否使用快捷连接shortcut，分组数g，扩展因子e。"""
        super().__init__()
        # # 初始化阈值 动态调整
        self.thresholds = nn.Parameter(torch.tensor([threshold1, threshold2]))  # 定义动态阈值参数
        # self.thresholds = [threshold1,threshold2]
        self.c = int(c2 * e)  # 隐藏通道数，根据扩展因子和输出通道数计算
        self.mca = PoolAttention(3)  # 创建池化通道
        ...
        # 当切分3份时
        self.cv31 = Conv(c1, 3 * self.c, 1, 1)
        self.cv32 = Conv((3 + n) * self.c, c2, 1)
        # 当切分4份时
        self.cv41 = Conv(c1, 4 * self.c, 1, 1)
        self.cv42 = Conv((4 + n) * self.c, c2, 1)
        self.splits = splits
    ...
    def forward(self, x):
        ...
        std = x.std().item()
        # 根据标准差和阈值确定分割数量
        if std < self.thresholds[0]:
            self.splits = 1
        elif self.thresholds[0] <= std < self.thresholds[1]:
            self.splits = 2
        elif self.thresholds[1] <= std:
            self.splits = 3
        if self.splits == 1:
            y = self.cv11(x)
            # y = self.cv12(y)
        elif self.splits == 2:
            y = list(self.cv21(x).chunk(self.splits, 1))
            y.extend(m(y[-1]) for m in self.m)
            y = self.cv22(torch.cat(y, 1))
        elif self.splits == 3:
            y = list(self.cv31(x).chunk(self.splits, 1))
            y.extend(m(y[-1]) for m in self.m)
            y = self.cv32(torch.cat(y, 1))
        return y
    ...
    def forward_split(self, x):
        ...
        if std < self.thresholds[0]:
            ...
            self.splits = 2
            split_sizes = [self.c for _ in range(self.splits)]  # 计算每个分割块的大小
            ...
            self.splits = 3
            split_sizes = [self.c for _ in range(self.splits)]  # 计算每个分割块的大小
            y = list(self.cv31(x).split(split_sizes, 1))
            y.extend(m(y[-1]) for m in self.m)
            y = self.cv32(torch.cat(y, 1))
        return y
